...
* *RCG-Integer-Dramatic-CRT* applies a custom shader preset based on some of Retro Game Corps' recommendations for a dramatic glow effect. This requires integer scaling in order to render evenly.
* *RCG-Noninteger-Dramatic-CRT* applies a custom shader preset based on some of Retro Game Corps' recommendations for a dramatic glow effect, but is intended to be used for systems without integer scaling.
